...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <asp:SqlDataSource ID="SqlDataSource4" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" 
        SelectCommand="SELECT PROJECT_ID, PROJECT_NAME, ROUND(PROJECT_DUE_DATE - SYSDATE) AS REMAINING_DAYS FROM PROJECTS WHERE PROJECT_STATUS = 'Ongoing' AND PROJECT_DUE_DATE >= SYSDATE ORDER BY REMAINING_DAYS ASC FETCH FIRST 7 ROWS ONLY"></asp:SqlDataSource>
    <asp:SqlDataSource ID="SqlDataSource5" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" 
        SelectCommand="SELECT (SELECT COUNT(DISTINCT user_id) FROM users) AS TotalUsers, (SELECT COUNT(DISTINCT project_id) FROM projects) AS TotalProjects, (SELECT COUNT(DISTINCT task_id) FROM tasks) AS TotalTasks, (SELECT COUNT(DISTINCT resource_id) FROM resources) AS TotalResources, (SELECT COUNT(DISTINCT subtask_id) FROM subtasks) AS TotalSubtasks FROM &quot;DUAL&quot;"></asp:SqlDataSource>
